=== PopMagique ===
Contributors: popmagique
Tags: popup, exit-intent, glassmorphism, conversion
Requires at least: 5.0
Tested up to: 6.4
Requires PHP: 7.4
Stable tag: 1.0.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Système de double popup intelligent avec design glassmorphism pour WordPress. Popup d'entrée publicitaire et popup de sortie promotionnel avec détection exit intent.

== Description ==

**PopMagique** est un plugin WordPress moderne qui vous permet d'augmenter vos conversions grâce à un système de double popup intelligent :

🎯 **Popup d'Entrée** - Affiche automatiquement une publicité ou une offre spéciale après 3 secondes
✋ **Popup de Sortie** - Détecte l'intention de quitter et met en avant votre offre

= Fonctionnalités Principales =

* **Design Glassmorphism** - Interface moderne avec effets de flou et transparence
* **Détection Exit Intent** - Technologie avancée pour détecter l'intention de quitter (desktop et mobile)
* **100% Responsive** - Optimisé pour tous les appareils
* **Interface en Français** - Plugin entièrement traduit
* **Configuration Intuitive** - Panneau d'administration simple et complet
* **Intégration Mailchimp** - Synchronisation automatique avec vos listes (optionnelle)
* **Personnalisation Complète** - Couleurs, polices (graisse, style, transformation), textes et délais configurables

= Cas d'Usage =

* **E-commerce** - Promouvoir des offres spéciales et réduire l'abandon de panier
* **Blogs** - Mettre en avant vos contenus avant la sortie du visiteur
* **Sites Vitrine** - Capturer des leads qualifiés
* **Landing Pages** - Maximiser les conversions

= Technologie =

* Détection exit intent avancée (mouvement souris, scroll, beforeunload)
* Animations CSS3 fluides et performantes
* Code JavaScript optimisé et non-intrusif
* Compatible avec tous les thèmes WordPress
* Respect des bonnes pratiques de sécurité WordPress

== Installation ==

1. Téléchargez le plugin depuis le répertoire WordPress ou uploadez le dossier `popmagique` dans `/wp-content/plugins/`
2. Activez le plugin via le menu 'Extensions' de WordPress
3. Allez dans 'PopMagique' dans votre menu d'administration
4. Configurez vos popups selon vos besoins
5. C'est tout ! Vos popups sont maintenant actifs

== Frequently Asked Questions ==

= Le plugin ralentit-il mon site ? =

Non, PopMagique est optimisé pour les performances. Les scripts sont minifiés et chargés de manière asynchrone.

= Puis-je désactiver un des deux popups ? =

Oui, vous pouvez activer/désactiver indépendamment le popup d'entrée et le popup de sortie.

= Le plugin fonctionne-t-il sur mobile ? =

Absolument ! PopMagique inclut une détection exit intent spécialement conçue pour les appareils mobiles.

= Puis-je personnaliser le design ? =

Oui, vous pouvez modifier les couleurs, polices, tailles, textes et même ajouter vos propres images.


== Screenshots ==

1. Interface d'administration - Configuration du popup d'entrée
2. Interface d'administration - Configuration du popup de sortie
3. Popup d'entrée avec design glassmorphism
4. Popup de sortie promotionnel
5. Aperçu responsive sur mobile

== Changelog ==

= 1.0.0 =
* Version initiale
* Popup d'entrée avec délai configurable
* Popup de sortie avec détection exit intent
* Design glassmorphism responsive
* Interface d'administration complète
* Support desktop et mobile

== Upgrade Notice ==

= 1.0.0 =
Version initiale de PopMagique. Installation recommandée pour tous les sites WordPress souhaitant augmenter leurs conversions.

== Support ==

Pour toute question ou problème :
* Documentation : https://popmagique.com/docs
* Support : https://popmagique.com/support
* GitHub : https://github.com/popmagique/wordpress-plugin

== Crédits ==

PopMagique utilise les technologies suivantes :
* jQuery (inclus avec WordPress)
* CSS3 pour les animations et effets glassmorphism
* API WordPress pour la gestion des options et AJAX
* Google Fonts (Inter) pour la typographie